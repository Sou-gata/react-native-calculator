...
import { addHistoryLog } from "../../helpers/functions";
    ...
    const calculate = () => {
        ...
        if (bmi && category) {
            const heightStr = unit === "Metric"
                ? `${height.m || "0"}m ${height.cm || "0"}cm`
                : `${height.ft || "0"}ft ${height.in || "0"}in`;
            const weightStr = unit === "Metric"
                ? `${weight.kg || "0"}kg ${weight.g || "0"}g`
                : `${weight.lb || "0"}lb ${weight.oz || "0"}oz`;
            addHistoryLog(
                "BMI Calculator",
